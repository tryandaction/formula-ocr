...
- 🎯 **高级公式检测** - 输出候选区域和检测状态；准确率需以人工基准评估
- 💎 **置信度过滤** (v2.1.1) - 交互式滑块过滤低质量检测，默认阈值0.75
...
- 📑 DOCX 暂不支持；请先转换为 PDF 或 Markdown
...
### 智能识别增强
- 🎯 公式类型选择（数学/物理/化学）作为识别请求提示
- 📊 图片质量检测（模糊、分辨率、对比度）
- 🔢 多公式分离检测
- 📈 识别置信度显示
- 🧠 **高级公式检测引擎** (v2.1.1) - 8组件模块化架构 + 深度优化
  - 图像预处理（去噪、增强、二值化）
  - 数学符号特征提取（希腊字母、积分、求和等）
  - 内容分类（公式/图片/表格/文本）+ 5条排除规则
  - 公式类型分类（独立/行内）
  - 精确边界检测
  - 多维度置信度评分
  - 智能缓存机制
  - 自动降级策略
  - **深度优化**：排除标题、作者、图片说明等误检
...
### 识别准确率如何？
当前没有足够人工 ground truth 支持准确率百分比；请以 `repair-baseline` 评估结果为准。

### 支持哪些类型的公式？
支持数学公式、物理公式、化学方程式等，包括分数、根号、积分、求和、矩阵等复杂符号。

### 文档解析功能支持哪些格式？
目前支持 PDF（最大 50MB，100 页）和 Markdown（最大 5MB）。DOCX 暂不支持，上传界面不会再宣称可直接解析。
